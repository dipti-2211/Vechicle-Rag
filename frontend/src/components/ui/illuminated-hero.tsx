...
export interface IlluminatedHeroProps {
  title?: string;
...
export function IlluminatedHero({
  title = "Introducing",
  highlightText = "Auron AI",
  subtitle = "The Vehicle AI Assistant.",
  description = "Experience intelligent vehicle diagnostics, document search, and real-time operational insights powered by advanced AI.",
  children
	...
	return (
		<div className="relative w-full flex h-screen flex-wrap items-center justify-center overflow-hidden bg-black text-[calc(var(--size)*0.022)] text-white [--factor:min(1000px,100vh)] [--size:min(var(--factor),100vw)]">
			...
			<div className="text-center text-4xl md:text-6xl font-semibold z-10" aria-hidden="true">
				{title}
				<br />
				<span
					className={cn(
			...
			<p className="absolute top-0 bottom-0 m-auto h-fit max-w-[28em] translate-y-[12em] bg-gradient-to-t from-[#86868b] to-[#bdc2c9] bg-clip-text text-center font-semibold text-transparent z-10 px-4 text-sm md:text-base">
				{description}
			</p>

			{children && (
				<div className="absolute bottom-16 left-0 right-0 z-20 flex justify-center items-center gap-4">
					{children}
				</div>
			)}
